...
def make_model(
    ...
    encoder_input = lbann.Identity(embeddings_slice,name='encoder_input')
    decoder_input = lbann.Identity(embeddings_slice,name='decoder_input')
    ...
    preds = lbann.Slice(preds, axis=0, slice_points=str_list(range(sequence_length)),name='pred_slice_sm')
...
def make_batch_script(trainer_params, model_params, script_params):
    ...
    opt = lbann.Adam(learn_rate=0.0001, beta1=0.9, beta2=0.98, eps=1e-8)
    ...
    model.callbacks.append(
        lbann.CallbackDropFixedLearningRate(
            drop_epoch= [4,8,16,32,64],#[4,16,64,128], #[2,4,8,12],
            amt=0.75,
        )
    )
